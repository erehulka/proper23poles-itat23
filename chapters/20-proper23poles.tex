...
By the Parity Lemma, in each colouring $\phi$ of a $5$-pole $M$, three semiedges of $M$ have the same colour. We call these edges \textit{sociable} in $\phi$. The remaining two semiedges of $M$, called \emph{solitary} in $\phi$, are colourd by the remaining two colours.
...
From a case analysis in \cite{Preissmann1983}, it follows that if both $M_1$ and $M_2$ are colourable, then for each $i \in \{1, 2\}$, the colouring graph $R_{M_i}$ of $M_i$ is a subgraph of:
\begin{itemize}
\item a $5$-cycle, or
...
The colouring graph of a superpentagon is either empty or the $5$-cycle. The colouring graph of a negator is empty, consists of one triangle or consists of two triangles with a common vertex.
